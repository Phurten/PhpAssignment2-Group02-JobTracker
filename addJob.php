...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($company_name) || empty($title) || empty($description) || empty($location) || empty($status) || empty($date_applied)) {
        ...
        //first, checking if company already exists
        ...
        if (mysqli_num_rows($company_result) > 0) {
            //if company exists, use existing ID
      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-md-10 col-lg-8">
            <h1 class="display-4 mt-5 mb-5 text-center">Add New Job Application</h1>
            <form action="addJob.php" method="POST">
              ...
              <button type="submit" class="btn hero-btn mt-3 d-block mx-auto" style="min-width: 300px;">Add Job</button>
            </form>
          </div>
        </div>
      </div>
